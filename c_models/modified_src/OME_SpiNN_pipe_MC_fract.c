// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/OME_SpiNN_pipe_MC_fract.c
...
void app_init(void)
{
    ...
    log_mini_info("%u%d", 8005,data_size);  /* "data_size=%d"*/
    log_mini_info("%u%d", 8006,TOTAL_TICKS);  /* "TOTAL_TICKS=%d"*/

    // Get a pointer to the data - not worth copying at present
    sdramin_buffer = (REAL *) &(params_enum[DATA]);//(REAL *) &params.DATA;//
    log_mini_info("%u%08x", 8007,(uint)sdramin_buffer);  /* "sdramin_buffer=0x%08x"*/
    ...
    log_mini_info("%u%d", 8008,r2s_key);  /* "r2s key=%d"*/
   ...
   // TIMER_TICK_PERIOD = (uint)(0.95*1e6 * ((REAL)SEGSIZE/Fs));//RT
    ...
    log_mini_info("%u%d", 8009,(uint)TIMER_TICK_PERIOD);  /* "timer period=%d"*/
    log_mini_info("%u%d", 8010,sampling_frequency);  /* "Fs=%d"*/
    final_ack=0;
...
void data_write(uint null_a, uint null_b)
{
	...
	if(test_DMA == TRUE)
	{
		...
		spin1_dma_transfer(DMA_WRITE,&sdramout_buffer[out_index],dtcm_buffer_out,DMA_WRITE,
		  						SEGSIZE*sizeof(REAL));
#ifdef PRINT
        log_mini_info("%u%d%d%08x", 8011, coreID,seg_index,(uint) &sdramout_buffer[out_index]);
...
void app_end(uint null_a,uint null_b)
{
    ...
    {
        log_mini_info("%u%d", 8012,seg_index);  /* "All data has been sent seg_index=%d"*/
...
void process_chan(REAL *in_buffer)
{
	...
	for(i=0;i<SEGSIZE;i++)
	{
        ...
        //if (ABS(concha) < 1e-14) concha = 0.0;//prevent small value calc performance drop
		//update vars
        ...
        //if (ABS(earCanalRes) < 1e-14) earCanalRes = 0.0;//prevent small value calc performance drop
		//update vars	
...
{
    sync_count++;
    log_mini_info("%u%d%d%d", 8013,mc_key-2,sync_count,seg_index);  /* "ack mcpacket recieved from key=%d sync_count=%d seg_index=%d\n"*/
}
