// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/OME_SpiNN_pipe_MC.c
...
void app_init(void)
{
    ...
    log_mini_info("%u%d", 8044,data_size);  /* "data_size=%d"*/
    log_mini_info("%u%d", 8045,TOTAL_TICKS);  /* "TOTAL_TICKS=%d"*/
    ...
    log_mini_info("%u%08x", 8046,(uint)sdramin_buffer);  /* "sdramin_buffer=0x%08x"*/
    ...
    log_mini_info("%u%d", 8047,r2s_key);  /* "r2s key=%d"*/

    rt=params.RT;
    log_mini_info("%u%d", 8048,rt);  /* "rt=%d"*/
    ...
    log_mini_info("%u%d", 8049,(uint)TIMER_TICK_PERIOD);  /* "timer period=%d"*/
    log_mini_info("%u%d", 8050,sampling_frequency);  /* "Fs=%d"*/
    final_ack=0;
...
/*
    ...
    log_mini_info("%u%k", 8051,(accum)stapesHP_b[0]);  /* "shp_b0:%k"*/
    log_mini_info("%u%k", 8052,(accum)stapesHP_b[1]);  /* "shp_b1:%k"*/
    log_mini_info("%u%k", 8053,(accum)stapesHP_b[2]);  /* "shp_b2:%k"*/
 log_mini_info("%u%k", 8054,(accum)stapesHP_a[0]);  /* "shp_a0:%k"*/
 log_mini_info("%u%k", 8055,(accum)stapesHP_a[1]);  /* "shp_a1:%k"*/
    log_mini_info("%u%k", 8056,(accum)stapesHP_a[2]);  /* "shp_a2:%k"*/
...
void data_write(uint null_a, uint null_b)
{
	...
	if(test_DMA == TRUE)
	{
		...
		spin1_dma_transfer(DMA_WRITE,&sdramout_buffer[out_index],dtcm_buffer_out,DMA_WRITE,
		  						SEGSIZE*sizeof(REAL));
#ifdef PRINT
        log_mini_info("%u%d%d%08x", 8057, coreID,seg_index,(uint) &sdramout_buffer[out_index]);
        /* "[core %d] segment %d written to @ 0x%08x\n"*/
...
void app_end(uint null_a,uint null_b)
{
    ...
    {
        log_mini_info("%u%d", 8058,seg_index);  /* "All data has been sent seg_index=%d"*/
...
{
    sync_count++;
    log_mini_info("%u%d%d%d", 8059,mc_key-2,sync_count,seg_index);  /* "ack mcpacket recieved from key=%d sync_count=%d seg_index=%d\n"*/
}
...
void app_done ()
{
  ...
  profiler_finalise();
#endif
 log_mini_info("%u%k", 8060,(accum)conchaFilter_b[0]);  /* "b0:%k"*/
    log_mini_info("%u%k", 8061,(accum)conchaFilter_b[1]);  /* "b1:%k"*/
    log_mini_info("%u%k", 8062,(accum)conchaFilter_b[2]);  /* "b2:%k"*/
 log_mini_info("%u%k", 8063,(accum)conchaFilter_a[0]);  /* "a0:%k"*/
 log_mini_info("%u%k", 8064,(accum)conchaFilter_a[1]);  /* "a1:%k"*/
    log_mini_info("%u%k", 8065,(accum)conchaFilter_a[2]);  /* "a2:%k"*/
