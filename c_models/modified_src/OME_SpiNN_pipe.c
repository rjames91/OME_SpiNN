// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/OME_SpiNN_pipe.c
...
void app_init(void)
{
    ...
    log_mini_info("%u%d", 8032,data_size);  /* "data_size=%d"*/
    log_mini_info("%u%d", 8033,TOTAL_TICKS);  /* "TOTAL_TICKS=%d"*/
    ...
    log_mini_info("%u%d", 8034,(uint)TIMER_TICK_PERIOD);  /* "timer period=%d"*/
    log_mini_info("%u%d", 8035,sampling_frequency);  /* "Fs=%d"*/
    final_ack=0;
...
void data_write(uint null_a, uint null_b)
{
  ...
  start_count_write = tc[T2_COUNT];
		...
		spin1_dma_transfer(DMA_WRITE,&sdramout_buffer[out_index],dtcm_buffer_out,DMA_WRITE,
		  						SEGSIZE*sizeof(REAL));
#ifdef PRINT
        log_mini_info("%u%d%d%08x", 8036, coreID,seg_index,(uint) &sdramout_buffer[out_index]);
...
void app_end(uint null_a,uint null_b)
{
    ...
    {
        log_mini_info("%u%d", 8037,seg_index);  /* "All data has been sent seg_index=%d"*/
  ...
  start_count_read = tc[T2_COUNT];
	...
	else if (read_ticks >= TOTAL_TICKS && sync_count==num_drnls)
	{
	                    ...
	                    //child models have replied with finished processing acks
	    {
            ...
            log_mini_info("%u%d%d", 8038,chipID,start_count_full-end_count_full);  /* "[Chip%d] total processing time=%d ticks"*/
...
void sync_check(uint key, uint payload)
{
    ...
    {
        sync_count++;
        log_mini_info("%u%d%d", 8039,sync_count,seg_index);  /* "ack mcpacket recieved sync_count=%d seg_index=%d\n"*/
    }
}
