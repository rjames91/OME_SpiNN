// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/OME_SpiNN_pipe_ack.c
...
void app_init(void)
{
    ...
    log_mini_info("%u%d", 8001,data_size);  /* "data_size=%d"*/
    log_mini_info("%u%d", 8002,TOTAL_TICKS);  /* "TOTAL_TICKS=%d"*/
...
{
   log_mini_info("%u%d", 8003,seg_index);  /* "All data has been sent and confirmed seg_index=%d"*/
...
void sync_check(uint key, uint payload)
{
    ...
    {
        sync_count++;
        log_mini_info("%u%d%d", 8004,sync_count,seg_index);  /* "ack mcpacket recieved sync_count=%d seg_index=%d\n"*/
    }
}
