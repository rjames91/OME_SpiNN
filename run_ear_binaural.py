...
for ear in range(num_channels):
    ...
    #plot_output_spikes(spike_trains,plotter=plt,markersize=1,color='black')'''
      ...
      #  plt.xlim((0,len(audio_data[ear])/Fs))
        ...
        # numpy.savez_compressed('./spinnakear_13.5_1_kHz_{}s_{}dB_vrr'.format(int(required_total_time), int(dBSPL)),
        #                        drnl=drnl,
        #                        onset_times=onset_times, dBSPL=dBSPL)
...
# Save the results
if bitfield:
    numpy.savez_compressed('./spinnakear_asc_des_{}s_{}dB'.format(int(duration),int(dBSPL)), scaled_times=binaural_scaled_times,
             onset_times=onset_times,dBSPL=dBSPL)
else:
    numpy.savez_compressed('./spinnakear_13.5_1_kHz_{}s_{}dB_vrr'.format(int(required_total_time), int(dBSPL)),
